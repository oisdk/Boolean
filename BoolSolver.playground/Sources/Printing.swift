...
extension Expr : CustomStringConvertible {
  public var description: String {
    ...
    case let .OR(.AND(a,b),.AND(c,d))
      ...
      let lhs = a   .precedence < 2 ? "(" + a   .description + ")" : a   .description
      let rhs = (!b).precedence < 2 ? "(" + (!b).description + ")" : (!b).description
      return lhs + " ⊻ " + rhs
    case let .NOT(.OR(.AND(a,b),.AND(c,d)))
      where a == !c && b == !d || a == !d && b == !c:
      let lhs = a   .precedence < 2 ? "(" + a   .description + ")" : a   .description
      let rhs = (!b).precedence < 2 ? "(" + (!b).description + ")" : (!b).description
      return "¬(" + lhs + " ⊻ " + rhs + ")"
    case let .NOT(.AND(a,b)):
